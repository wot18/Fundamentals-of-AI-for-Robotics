...
def default_search(start_pos, goal_pos, obstacles, grid_size=0.5):
    ...
    start_node.h_cost = default_heuristic(start_pos, goal_pos)
    ...
    while open_list:
        ...
        if default_heuristic(current.position, goal_pos) < grid_size:
            path = []
            while current:
        ...
        # 修改这里，传入goal_pos参数
        for neighbor_pos in get_neighbors(current.position, goal_pos, grid_size, heuristic_func=default_heuristic):
            if any(default_heuristic(neighbor_pos, obs) < grid_size for obs in obstacles):
                continue
                
            g_cost = current.g_cost + default_heuristic(current.position, neighbor_pos)
            ...
            if g_cost < neighbor.g_cost:
                ...
                neighbor.h_cost = default_heuristic(neighbor_pos, goal_pos)
